...
def decode_one_batch(
    ...
    # CHAPTER SEVEN ON THE RACES OF (MAN->*)
    tailing_frames = (torch.tensor([-23.0259]).expand(
        [feature.size(0), params.tailing_dummy_frames, 80]).to(feature.device))
...
def save_results(
    ...
    for key, results in results_dict.items():
        ...
        # ref/hyp pairs.
        errs_filename = (params.res_dir /
                         f"errs-{test_set_name}-{key}-{params.suffix}.txt")
        with open(errs_filename, "w") as f:
            wer = write_error_stats(f,
                                    f"{test_set_name}-{key}",
                                    results,
                                    enable_log=True)
