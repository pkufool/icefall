...
from solver import DPMSolver, EulerSolver
...
def get_distill_model(params: AttributeDict) -> nn.Module:
    ...
    params.use_guidance_scale_embed = True
    fm_decoder = get_fm_decoder_model(params)
    text_encoder = get_text_encoder_model(params)
...
def get_fm_decoder_model(params: AttributeDict) -> nn.Module:
    ...
    encoder = TTSZipformer(
        ...
        encoder_dim=to_int_tuple(params.fm_decoder_dim),
        feedforward_dim=to_int_tuple(params.fm_decoder_feedforward_dim),
        ...
        use_guidance_scale_embed=(
            params.use_guidance_scale_embed
            if "use_guidance_scale_embed" in params
            else False
        ),
    )
    return encoder
...
def get_text_encoder_model(params: AttributeDict) -> nn.Module:
    ...
    encoder = TTSZipformer(
        ...
        encoder_dim=to_int_tuple(params.text_encoder_dim),
        feedforward_dim=to_int_tuple(params.text_encoder_feedforward_dim),
...
class TtsModel(nn.Module):
    ...
    def __init__(
        ...
        """
        Args:
            fm_decoder: the audio encoder model, inputs are the input condition
                embeddings and noisy acoustic features, outputs are better acoustic features.
            text_encoder: the text encoder model. input are text embeddings, output are contextualized
                text embeddings.
    ...
    def forward_audio(
        ...
        """Compute velocity.
        Args:
            t:  A tensor of shape (N, 1, 1) or a tensor of a float, in the range of (0, 1).
            xt: the input of the current timestep, including condition embeddings
                and noisy acoustic features.
            text_condition: the text condition embeddings, with the shape (batch, seq_len, emb_dim).
            audio_condition: the audio condition embeddings, with the shape (batch, seq_len, emb_dim).
            padding_mask: The mask for padding, True means masked position, with the shape (N, T).
            guidance_scale: The guidance scale in classifier-free guidance, which is
                a tensor of shape (N, 1, 1) or a tensor of a float.
        ...
        xt = torch.cat([xt, text_condition, audio_condition], dim=2)
        vt = self.fm_decoder(
    ...
    def forward_text_condition(
        ...
        Returns:
            text_condition: the text condition, shape (batch, feature_seq_len, emb_dim).
            padding_mask: the padding mask of text condition, shape (batch, feature_seq_len).
        """
    ...
    def forward_text_inference_ratio_duration(
        ...
        features_lens = torch.ceil(
            (prompt_features_lens / prompt_tokens_lens * cat_tokens_lens)
    ...
    def forward(
        ...
        audio_condition = torch.where(audio_condition_mask.unsqueeze(-1), 0, features)
    ...
    def sample(
        ...
        """
        Sample audios, given text tokens and prompts.
        Args:
            tokens: a list of list of text tokens.
            prompt_tokens: a list of list of prompt tokens.
            prompt_features: the prompt feature with the shape (batch_size, seq_len, feat_dim).
            prompt_features_lens: the length of each prompt feature, with the shape (batch_size,).
            features_lens: the length of the predicted feature, with the shape (batch_size,).
                It is used only when duration is "real".
            duration: "real" or "predict". If "real", the predicted feature length
                is given by features_lens.
            solver: "dpm" or "euler". The ODE solver to use.
        ...
        audio_condition = torch.nn.functional.pad(
            prompt_features, (0, 0, 0, num_frames - prompt_features.size(1))
        )  # (B, T, F)

        # False means audio condition positions.
        audio_condition_mask = make_pad_mask(prompt_features_lens, num_frames)
        audio_condition = torch.where(
            audio_condition_mask.unsqueeze(-1),
            torch.zeros_like(audio_condition),
            audio_condition,
        )
        ...
        if solver == "dpm" and num_step > 2:
            solver = DPMSolver(self, distill=distill, func_name="forward_audio")
        else:
            solver = EulerSolver(self, distill=distill, func_name="forward_audio")
    ...
    def sample_intermediate(
        ...
        features_lens: torch.Tensor,
        noise: torch.Tensor,
        audio_condition_mask: torch.Tensor,
        ...
        """
        Sample audio in intermediate timesteps.
        Args:
            tokens: List of list of token ids.
            features: The acoustic features, with the shape (batch, seq_len, feat_dim).
            features_lens: The length of each acoustic feature sequence, with the shape (batch,).
            noise: The initial noise, with the shape (batch, seq_len, feat_dim).
            audio_condition_mask: The mask for audio condition, True means non-condition positions,
                with the shape (batch, seq_len).
            t_start: The start timestep, with the shape (batch, 1, 1).
            t_end: The end timestep, with the shape (batch, 1, 1).
            solver: The solver for sampling, can be "euler" or "dpm".
            num_step: The number of steps for sampling.
            guidance_scale: The scale for text-to-audio guidance, with the shape (batch, 1, 1).
